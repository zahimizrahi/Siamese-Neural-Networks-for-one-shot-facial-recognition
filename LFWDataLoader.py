...
class LFWDataLoader():
    ...
    #                  PREPROCESSING UTILS
    # -----------------------------------------------------------
    def image2tensor(self, img_raw, norm=None, _resize=None):
        """
    ...
    def load_image_as_tensor(self, img_path, norm=None, _resize=None):
        """
        ...
        return self.image2tensor(img_raw, norm=norm, _resize=_resize)

    def load_images_as_tensor_with_label(self, paths, label, norm=None, _resize=None):
        """
        ...
        """
        left_img = self.load_image_as_tensor(paths[0], norm=norm, _resize=_resize)
        right_img = self.load_image_as_tensor(paths[1], norm=norm, _resize=_resize)
    ...
    def create_tensor_dataset(self, img_paths=None, labels=None, norm=None, _resize=None):
        """
        ...
        if img_paths is not None and labels is not None:
            ...
            tensor_data = tensor_data.map(lambda paths, label: self.load_images_as_tensor_with_label(paths, label, norm=norm, _resize=_resize),
                                          num_parallel_calls=tf.data.experimental.AUTOTUNE)
        elif img_paths is not None:  # image dataset
            tensor_data = tf.data.Dataset.from_tensor_slices(img_paths)
            tensor_data = tensor_data.map(lambda paths: self.load_image_as_tensor(paths, norm=norm, _resize=_resize),
                                          num_parallel_calls=tf.data.experimental.AUTOTUNE)
        elif labels is not None:  # label dataset
            tensor_data = tf.data.Dataset.from_tensor_slices(tf.cast(labels, tf.bool))
        return tensor_data

    def prepare_tensor_dataset(self, tensor_data, batch_size, buffer_size):
        """
    ...
    def init_tensor_data(self, tensor_data, images_labels_path, norm=None, _resize=None, batch_size=1, buffer_size=None,
                         verbose=True):
        """
        ...
        if tensor_data is None:
            ...
            tensor_data = self.create_tensor_dataset(img_paths=images_paths, labels=labels, norm=norm, _resize=_resize)
            tensor_data = self.prepare_tensor_dataset(tensor_data, batch_size=batch_size, buffer_size=buffer_size)
            if verbose:
                print(tensor_data)
